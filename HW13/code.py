...
def webpage(dispTime):
    ...
    <p><form accept-charset="utf-8" method="POST">
    <button class="button" name="STOPWATCH" value="START" type="submit">STOPWATCH</button></a></p></form>
    <p class="dotted">The time since the last button press is
    <span style="color: deeppink;">{dispTime}</span></p><br>
    </body></html>
    """
    return html
...
def buttonpress(request: HTTPRequest):
    ...
    if "START" in raw_text:
        #  display time since last button press
        dispTime = time.monotonic() - t
        t = time.monotonic()
        
    #  reload site
    with HTTPResponse(request, content_type=MIMEType.TYPE_HTML) as response:
        response.send(f"{webpage(dispTime)}")
