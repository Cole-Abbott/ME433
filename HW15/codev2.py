...
while True:
    ...
    # send the line to the computer

    # find average brightness on each edge of the line
    edgeAvg = 0
    for i in range(0,10):
        edgeAvg += (bright[i] + bright[59-i]) / 20 
    ...
    for i in range(10,50):
        # find the average brightness in the middle
        midAvg = sum(bright[i:i+10])/10
        # find the difference between the middle and the edges
        diff = abs(midAvg - edgeAvg)
